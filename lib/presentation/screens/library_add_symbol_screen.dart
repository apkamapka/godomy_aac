...
class LibraryAddSymbolScreen extends ConsumerStatefulWidget {
  const LibraryAddSymbolScreen({super.key});

  @override
  ConsumerState<LibraryAddSymbolScreen> createState() => _LibraryAddSymbolScreenState();
}

class _LibraryAddSymbolScreenState extends ConsumerState<LibraryAddSymbolScreen> {
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      final newSymbol = LibrarySymbolModel(
        id: const Uuid().v4(),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.symbolCreated), backgroundColor: Colors.green));
        context.pop();
      }
    } catch (e) {
      if (mounted) {
